...
{\tt disable=[arguments|comet|retransmit]+}
...
\begin{description}
\item[Plugin] The first argument is the plugin, implementing the desired link-layer protocol. Currently, the two existing plugins are SLIP (Serial Line IP) and APDUIP (APDU supporting IP);
...
PanManager configuration (as root):
\begin{verbatim}
$ panManager/bin/panManager slip 192.168.1.1/24
\end{verbatim}
