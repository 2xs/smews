...
int  utun_open  (utun *t, char *ip_src, char *ip_dest,unsigned long mtu)
{
  ...
  for (i = 0; ; i++) 
  {
    ...
    if(RegEnumKeyEx(key, i, (LPSTR)adapterid, &len, 0, 0, 0, NULL))
      break;

    /* Find out more about this adapter */

    sprintf_s(regpath, sizeof(regpath), "%s\\%s\\Connection", NETWORK_CONNECTIONS_KEY, adapterid);
    ...
    sprintf_s(tapname, sizeof(tapname), USERMODEDEVICEDIR "%s" TAPSUFFIX, adapterid);
  ...
  sprintf_s(cmd, sizeof(cmd), "netsh interface ip set address \"%s\" static %s 255.255.255.252",adaptername,ip_src);
#ifdef _DEBUG
  ...
  {
    sprintf_s(cmd, sizeof(cmd),"netsh interface ip show config \"%s\"",adaptername);
  ...
  if(mtu >= 100 && mtu <= 1500)// TAP-WIN32 V8 constraint
  {
      ...
      _itoa_s(mtu,str_mtu,sizeof(str_mtu),10);
      ...
      for (i = 0; ; i++) 
      {
        DWORD len = sizeof(keyid);
        if(RegEnumKeyEx(key, i, (LPSTR)keyid, &len, 0, 0, 0, NULL))
          break;

        sprintf_s(regpath, sizeof(regpath), "%s\\%s\\", ADAPTER_KEY, keyid);
